...
class MLPRegressorCircular(nn.Module):
    """Define PyTorch MLP Model to handle longitude and latitude predictions."""
    ...
    def __init__(
        ...
        super(MLPRegressorCircular, self).__init__()
        self.device = device
        self.dtype = dtype
        ...
        # Adjust output_width to 4 (2 for longitude and 2 for latitude)
        self.seqmodel = self._define_model(
            input_size, width, nlayers, dropout_prop, output_width
        )
    ...
    def _define_model(self, input_size, width, nlayers, dropout_prop, output_width):
        ...
        # Add output layers with 'tanh' activation for the final layer
        layers.append(nn.Linear(width, output_width, dtype=self.dtype))
